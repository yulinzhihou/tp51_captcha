...
class Captcha
{
     ...
     */
    public function check($code,$name,$id = '')
    {
        $key = $this->authcode($this->seKey) . $id;
        //接口验证码验证
        $captcha = Cache::get($name);
        if (empty($captcha['verify_code'])) {
            //表示验证码已经过期
            return false;
        }
        //接口验证
     ...
     * @access public
     * @param string $browser 浏览器类型。默认为IE,
     * @param string $id 要生成验证码的标识
     * @return \think\Response
     */
    public function entry($browser='ie',$id = '')
    {
        // 图片宽(px)
...
//        $code                  = $this->authcode(strtoupper(implode('', $code)));
        $secode                = [];
        $secode['verify_code'] = implode('', $code); // 把校验码保存到session
        $secode['create_time'] = time(); // 验证码创建时间
        $secode['verify_time'] = time()+$this->expire; // 验证码创建时间
